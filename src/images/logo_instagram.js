import React from "react"

const LogoInstagram = ({ baseLayer, color1 }) => (
  <svg
    className={baseLayer}
    version="1.1"
    id="Layer_1"
    x="0px"
    y="0px"
    viewBox="0 0 512 512"
  >
    <path
      className={color1}
      d="M349.3,69.3c51.5,0.2,93.2,41.9,93.3,93.3v186.7c-0.2,51.5-41.9,93.2-93.3,93.3H162.7c-51.5-0.2-93.2-41.9-93.3-93.3V162.7
	c0.2-51.5,41.9-93.2,93.3-93.3H349.3 M349.3,32H162.7C90.8,32,32,90.8,32,162.7v186.7C32,421.2,90.8,480,162.7,480h186.7
	c71.9,0,130.7-58.8,130.7-130.7V162.7C480,90.8,421.2,32,349.3,32z"
    />
    <path
      d="M377.3,162.7c-15.5,0-28-12.5-28-28s12.5-28,28-28s28,12.5,28,28c0,15.4-12.4,28-27.9,28
	C377.4,162.7,377.4,162.7,377.3,162.7z"
    />
    <path
      d="M256,181.3c41.2,0,74.7,33.4,74.7,74.7s-33.4,74.7-74.7,74.7s-74.7-33.4-74.7-74.7C181.4,214.8,214.8,181.4,256,181.3
	 M256,144c-61.9,0-112,50.1-112,112s50.1,112,112,112s112-50.1,112-112S317.9,144,256,144z"
    />
  </svg>
)

export default LogoInstagram
